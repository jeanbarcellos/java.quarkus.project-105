 ...
 */
public class TestLocalDateDeserializer extends JSR310DateTimeDeserializerBase<LocalDate> {

    private static final long serialVersionUID = 1L;

    private static final DateTimeFormatter DEFAULT_FORMATTER = DateTimeFormatter.ISO_LOCAL_DATE;

    public static final TestLocalDateDeserializer INSTANCE = new TestLocalDateDeserializer();

    protected TestLocalDateDeserializer() {
        this(DEFAULT_FORMATTER);
    }

    public TestLocalDateDeserializer(DateTimeFormatter dtf) {
     ...
     * Since 2.10
     */
    public TestLocalDateDeserializer(TestLocalDateDeserializer base, DateTimeFormatter dtf) {
        super(base, dtf);
    }
     ...
     * Since 2.10
     */
    protected TestLocalDateDeserializer(TestLocalDateDeserializer base, Boolean leniency) {
        super(base, leniency);
    }
     ...
     * Since 2.11
     */
    protected TestLocalDateDeserializer(TestLocalDateDeserializer base, JsonFormat.Shape shape) {
        super(base, shape);
    }

    @Override
    protected TestLocalDateDeserializer withDateFormat(DateTimeFormatter dtf) {
        return new TestLocalDateDeserializer(this, dtf);
    }

    @Override
    protected TestLocalDateDeserializer withLeniency(Boolean leniency) {
        return new TestLocalDateDeserializer(this, leniency);
    }

    @Override
    protected TestLocalDateDeserializer withShape(JsonFormat.Shape shape) {
        return new TestLocalDateDeserializer(this, shape);
    }
    ...
    public LocalDate deserialize(JsonParser parser, DeserializationContext context) throws IOException {
        ...
        if (parser.hasToken(JsonToken.VALUE_NUMBER_INT)) {
            // issue 58 - also check for NUMBER_INT, which needs to be specified when
            // serializing.
            var x = isLenient();
            if (_shape == JsonFormat.Shape.NUMBER_INT || x) {
